...
export const apiJourneyRepository: JourneyRepository = {
  ...
  async moveItem(journeyId, itemId, direction) {
    const journey = await request<Journey>(`/journeys/${journeyId}`)
  ...
  startEditSession: (journeyId, password) => request<void>(`/journeys/${journeyId}/edit-session`, { method: 'POST', body: JSON.stringify({ password }) }),
