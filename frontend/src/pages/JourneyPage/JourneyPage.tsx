...
import { journeyRepository } from '../../repositories/mockJourneyRepository'
...
type Editor = { type: 'event'; itemId?: string; index?: number } | { type: 'transit'; index?: number } | { type: 'detail'; itemId: string } | null
    ...
    <main className={styles.page}>
      ...
      <div className={`${styles.workspace} ${!isEditing ? styles.viewWorkspace : ''}`}>
          ...
          {groups.length === 0 ? <div className={styles.emptyTimeline}><span>○</span><h2>まだ予定がありません</h2><p>旅の最初の目的地を追加しましょう。</p>{isEditing && <button type="button" onClick={() => setEditor({ type: 'event' })}>予定を追加する</button>}</div> : groups.map((group) => (
            <section className={styles.day} key={group.key}><h2>{group.label}</h2><div className={styles.timelineLine}>{group.items.map((item) => (
              <div className={styles.itemWrap} key={item.id}>
                <TimelineCard item={item} selected={selectedItem?.id === item.id} onClick={() => setEditor(isEditing && item.item_type === 'event' ? { type: 'event', itemId: item.id } : { type: 'detail', itemId: item.id })} />
        ...
        <aside className={styles.detailPane}>
          {!editor && <JourneyEditor journey={journey} editable={isEditing} onSaved={(updated) => { setJourney(updated); showNotice('旅程名を保存しました') }} />}
          ...
          {editor?.type === 'transit' && <TransitEditor onCancel={() => setEditor(null)} onSave={async (route, from, to) => { setBusy(true); await journeyRepository.createTransit(journey.id, { route, departure_location: from, arrival_location: to }, editor.index); await refresh(); setEditor(null); setBusy(false); showNotice('移動を追加しました') }} busy={busy} />}
...
function CreateJourney({ onCreated }: { onCreated: (journey: Journey) => void }) {
  ...
  const [busy, setBusy] = useState(false)
  const submit = async (event: FormEvent) => { event.preventDefault(); if (!title.trim() || !password) return; setBusy(true); onCreated(await journeyRepository.createJourney(title.trim())) }
...
function JourneyEditor({ journey, editable, onSaved }: { journey: Journey; editable: boolean; onSaved: (journey: Journey) => void }) {
  ...
  return <div className={styles.editor}><p className={styles.sectionLabel}>JOURNEY</p><h2>{editable ? '旅程の設定' : '旅程について'}</h2><label>タイトル<input value={title} disabled={!editable} onChange={(event) => setTitle(event.target.value)} /></label>{editable && <button className={styles.saveButton} type="button" disabled={!title.trim() || title === journey.title} onClick={async () => onSaved(await journeyRepository.updateJourney(journey.id, title.trim()))}>変更を保存</button>}<div className={styles.editorNote}><span>⌁</span><p><strong>モックモード</strong><br />変更内容はこのブラウザに保存されます。</p></div></div>
}
...
function TransitEditor({ onCancel, onSave, busy }: { onCancel: () => void; onSave: (route: TransitRoute, from: string, to: string) => void; busy: boolean }) {
  const [from, setFrom] = useState(''); const [to, setTo] = useState(''); const [time, setTime] = useState(''); const [routes, setRoutes] = useState<TransitRoute[]>([]); const [searching, setSearching] = useState(false)
  ...
  return <div className={styles.editor}><div className={styles.editorTop}><div><p className={styles.sectionLabel}>TRANSIT</p><h2>移動を追加</h2></div><button className={styles.close} type="button" onClick={onCancel}>×</button></div><form onSubmit={search}><label>出発地<input autoFocus value={from} onChange={(event) => setFrom(event.target.value)} placeholder="新浦安駅" required /></label><label>到着地<input value={to} onChange={(event) => setTo(event.target.value)} placeholder="秋葉原駅" required /></label><label>出発日時<input type="datetime-local" value={time} onChange={(event) => setTime(event.target.value)} /></label><button className={styles.searchButton} type="submit" disabled={searching || !from.trim() || !to.trim()}>{searching ? '検索中…' : '経路を検索'}</button></form>{routes.length > 0 && <div className={styles.routes}><p className={styles.sectionLabel}>ROUTE OPTIONS</p>{routes.map((route) => <button type="button" key={route.id} disabled={busy} onClick={() => onSave(route, from, to)}><span><strong>{route.summary}</strong><small>{formatTime(route.departure_time)} → {formatTime(route.arrival_time)}</small></span><span><strong>{route.duration_minutes}分</strong><small>¥{route.total_fare} · 乗換{route.transfers_count}回</small></span></button>)}</div>}</div>
}
...
function AuthDialog({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  ...
  const submit = (event: FormEvent) => { event.preventDefault(); if (password === 'demo') onSuccess(); else setError('パスワードが違います。モックでは「demo」です。') }
