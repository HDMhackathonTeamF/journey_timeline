    ...
    <main className={styles.historyPage}>
      ...
      <section className={styles.historyContent}>
        ...
        {loading ? <div className={styles.loading}>旅程を読み込んでいます…</div> : <div className={styles.grid}>{journeys.map((journey, index) => (
          <article className={styles.card} key={journey.id}><button type="button" onClick={() => navigate(`/journeys/${journey.id}`)}><span className={styles.cardNumber}>{String(index + 1).padStart(2, '0')}</span><span className={styles.cardDates}>{formatShortDate(journey.start_date)} — {formatShortDate(journey.end_date)}</span><strong style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 17px', alignItems: 'center', gap: 10, minWidth: 0 }}><span style={{ overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>{journey.title}</span><span title={journey.is_protected ? '編集パスワードあり' : undefined} aria-label={journey.is_protected ? '編集パスワードあり' : undefined} aria-hidden={!journey.is_protected} style={{ display: 'inline-flex', width: 17, color: 'var(--ink)', visibility: journey.is_protected ? 'visible' : 'hidden' }}><LockIcon /></span></strong><span className={styles.cardMeta}>{journey.item_count} stops · 更新 {formatUpdated(journey.updated_at)}</span><span className={styles.cardArrow}>旅程を見る →</span></button></article>
        ))}</div>}
